...
AUX_RGBImageRec *LoadBMP(char *filename)			// 载入位图图象
{
	...
	if (file) {										// 文件存在么?
		fclose(file);								// 关闭句柄
		return auxDIBImageLoad(TEXT("barbara.bmp"));  // 载入位图并返回指针
	}
...
int LoadGLTextures()								// 载入位图(调用上面的代码)并转换成纹理
{
	...
	if (textureImage[0] = LoadBMP("barbara.bmp")) {		// 载入位图，检查有无错误，如果位图没找到则退出
		status = TRUE;									// 将 Status 设为 TRUE
		glGenTextures(1, &texture[0]);					// 创建纹理
		glBindTexture(GL_TEXTURE_2D, texture[0]);		// 使用来自位图数据生成 的典型纹理
		glTexImage2D(GL_TEXTURE_2D, 0, 3, textureImage[0]->sizeX,
			textureImage[0]->sizeY, 0, GL_RGB, GL_UNSIGNED_BYTE, textureImage[0]->data);  // 生成纹理
...
int DrawGLScene(GLvoid)  // 从这里开始进行所有的绘制
{
	...
	*/
	...
	// 画个正方体
	{
		...
		glTexCoord2f(0.0f, 0.0f); glColor3f(1.0f, 0.0f, 0.0f); glVertex3f(-1.0f, 1.0f, 1.0f);    // 1
		glTexCoord2f(1.0f, 0.0f); glColor3f(1.0f, 1.0f, 0.0f); glVertex3f(1.0f, 1.0f, 1.0f);	   // 2
		glTexCoord2f(1.0f, 1.0f); glColor3f(1.0f, 1.0f, 1.0f); glVertex3f(1.0f, 1.0f, -1.0f);    // 3
		glTexCoord2f(0.0f, 1.0f); glColor3f(0.0f, 0.0f, 1.0f); glVertex3f(-1.0f, 1.0f, -1.0f);   // 4

		glTexCoord2f(0.0f, 0.0f); glColor3f(1.0f, 0.0f, 0.0f); glVertex3f(-1.0f, 1.0f, 1.0f);    // 1
		glTexCoord2f(1.0f, 0.0f); glColor3f(1.0f, 0.0f, 0.0f); glVertex3f(-1.0f, -1.0f, 1.0f);   // 5
		glTexCoord2f(1.0f, 1.0f); glColor3f(1.0f, 1.0f, 0.0f); glVertex3f(1.0f, -1.0f, 1.0f);	   // 6
		glTexCoord2f(0.0f, 1.0f); glColor3f(1.0f, 1.0f, 0.0f); glVertex3f(1.0f, 1.0f, 1.0f);	   // 2

		glTexCoord2f(0.0f, 0.0f); glColor3f(1.0f, 1.0f, 0.0f); glVertex3f(1.0f, 1.0f, 1.0f);	   // 2
		glTexCoord2f(1.0f, 0.0f); glColor3f(1.0f, 1.0f, 0.0f); glVertex3f(1.0f, -1.0f, 1.0f);	   // 6
		glTexCoord2f(1.0f, 1.0f); glColor3f(1.0f, 1.0f, 1.0f); glVertex3f(1.0f, -1.0f, -1.0f);   // 7
		glTexCoord2f(0.0f, 1.0f); glColor3f(1.0f, 1.0f, 1.0f); glVertex3f(1.0f, 1.0f, -1.0f);    // 3
...
{
	MessageBox(NULL, TEXT("左边是问号"), TEXT("ok_cancel"), MB_OKCANCEL | MB_ICONQUESTION);
	int result = MessageBox(NULL, TEXT("左边是感叹号"), TEXT("Yes_No"), MB_ICONINFORMATION | MB_YESNO);
	switch (result)/*注意！使用Unicode应用TEXT包围字串*/
	{
		case IDYES:MessageBox(NULL, TEXT("选择了YES"), TEXT("YES"), MB_OK); break;
		case IDNO:MessageBox(NULL, TEXT("选择了NO"), TEXT("NO"), MB_OK); break;
	}
